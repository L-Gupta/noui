...
from backend.elicitation.database import Base
...
class Narration(Base):
    ...
    capture_session_id: Mapped[str | None] = mapped_column(
        ForeignKey("capture_sessions.id", ondelete="CASCADE"), nullable=True
    )
    content: Mapped[str] = mapped_column(Text)
    url: Mapped[str] = mapped_column(String(2000), default="")
    timestamp: Mapped[datetime] = mapped_column(default=_utcnow)
...
# ClickEvent is defined in backend.shared.models — import it from there
from backend.shared.models import ClickEvent as ClickEvent  # noqa: F401, E402
...
class CaptureLoginDraft(Base):
    """Tabby bundle generated from a capture session (elicitation path)."""

    __tablename__ = "elicitation_login_drafts"
