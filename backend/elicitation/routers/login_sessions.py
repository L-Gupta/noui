...
async def analyze_login_session(
    ...
    Stores the result in CaptureLoginDraft and returns the bundle.
    ...
    # Store or update CaptureLoginDraft
    existing = await db.execute(
        select(CaptureLoginDraft).where(CaptureLoginDraft.capture_session_id == session_id)
    )
