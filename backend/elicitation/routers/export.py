...
async def export_project(project_id: str, db: AsyncSession = Depends(get_db)):
    ...
    processes = (
        (await db.execute(select(Process).where(Process.project_id == project_id))).scalars().all()
    )
    messages = (
        (await db.execute(select(Message).where(Message.project_id == project_id))).scalars().all()
    )
    narrations = (
        (await db.execute(select(Narration).where(Narration.project_id == project_id)))
        .scalars()
        .all()
    )
    timeline_events = (
        (await db.execute(select(TimelineEvent).where(TimelineEvent.project_id == project_id)))
        .scalars()
        .all()
    )
    click_events = (
        (await db.execute(select(ClickEvent).where(ClickEvent.project_id == project_id)))
        .scalars()
        .all()
    )
    capture_sessions = (
        (await db.execute(select(CaptureSession).where(CaptureSession.project_id == project_id)))
        .scalars()
        .all()
    )
    screenshots = (
        (await db.execute(select(Screenshot).where(Screenshot.project_id == project_id)))
        .scalars()
        .all()
    )
    documents = (
    ...
    attachments = (
        (await db.execute(select(Attachment).where(Attachment.project_id == project_id)))
        .scalars()
        .all()
    )
    questions = (
        (await db.execute(select(Question).where(Question.project_id == project_id)))
        .scalars()
        .all()
    )
    ...
    return {
        ...
        "capture_sessions": [
            CaptureSessionOut.model_validate(cs).model_dump(mode="json") for cs in capture_sessions
        ],
        "screenshots": [
            ScreenshotOut.model_validate(s).model_dump(mode="json") for s in screenshots
        ],
