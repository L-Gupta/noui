...
@router.get("/capture-sessions", response_model=list[CaptureSessionOut])
async def list_all_capture_sessions(db: AsyncSession = Depends(get_db)):
    stmt = select(CaptureSession).order_by(CaptureSession.started_at.desc().nullslast())
    result = await db.execute(stmt)
    return result.scalars().all()
