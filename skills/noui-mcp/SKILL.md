...
Manage generated FastMCP servers and connect them to Claude Code. Servers are generated by `/noui-record-workflow` and live under `mcp_servers/<app_slug>/<server_id>/`.

All commands run from the `noui/` directory using `.venv/bin/python3.12 cli/main.py`.

**Prerequisite:** At least one server must exist under `mcp_servers/`. Run `/noui-record-workflow` first if none exist.
