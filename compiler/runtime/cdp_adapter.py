"""Generate the noui_runtime/cdp.py source code for a compiled MCP server or Skill.
...
authenticated browser via Chrome DevTools Protocol (CDP). Cookies ride on
`credentials: 'include'`; the real browser's TLS fingerprint is preserved.

Port 9222 (direct CDP) is required; port 9223 is Tabby's relay and whitelists
only `Page.screencast*` / `Input.dispatch*Event`, which blocks `Runtime.evaluate`.
"""

from __future__ import annotations


def generate_cdp_adapter() -> str:
    """Generate noui_runtime/cdp.py source code as a string.

    The generated module provides:
      - find_page(domain)                      — locate a Tabby page target
      - cdp_eval(ws_url, expression)           — run JS and return parsed result
      - cdp_fetch(ws_url, url, method, body, headers) — fetch inside the browser

    Returns:
        Python source code string for noui_runtime/cdp.py.
    """
    return '''\
